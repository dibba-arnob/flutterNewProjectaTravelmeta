...
class _FlightsContentState extends State<FlightsContent> {
  ...
  Future<void> _showBookingDialog(Flight flight) async {
  ...
  void _proceed() {
    ...
    Navigator.pop(
      ...
      BookingPayload(
        ...
        subtitle:
            '${flight.fromCode} → ${flight.toCode} · ${flight.cabinClass.isEmpty ? 'Economy' : flight.cabinClass}',
        quantitySummary:
            '$_passengers Passenger${_passengers > 1 ? 's' : ''}',
        checkInLabel: 'DEPARTURE',
        checkInValue: _fmtDate(flight.departsAt),
        guestsLabel: 'PASSENGERS',
        guestsValue: '$_passengers Adult${_passengers > 1 ? 's' : ''}',
        serviceIcon: Icons.flight_rounded,
        serviceLabel: 'Flight',
        accentColor: AppColors.secondary,
      ),
    );
